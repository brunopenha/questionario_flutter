...
void main() {
  ...
  test('Deveria retornar um erro se o valor estiver vazio', () {
    expectLater(sut.valida({'qualquer_campo': ''}), ErroValidacao.DADO_INVALIDO);
  });

  test('Deveria retornar um erro se o valor estiver nulo', () {
    expectLater(sut.valida({'qualquer_campo': null}), ErroValidacao.DADO_INVALIDO);
  });

  test('Deveria retornar um erro se o valor abaixo do tamanho minimo', () {
    expectLater(
        sut.valida({'qualquer_campo': faker.randomGenerator.string(4, min: 1)}), ErroValidacao.DADO_INVALIDO);
  });

  test('Deveria retornar um erro se o valor igual ao tamanho minimo', () {
    expectLater(sut.valida({'qualquer_campo': faker.randomGenerator.string(5, min: 5)}), null);
  });

  test('Deveria retornar um erro se o valor mairo que o tamanho minimo', () {
    expectLater(sut.valida({'qualquer_campo': faker.randomGenerator.string(10, min: 6)}), null);
  });
}
